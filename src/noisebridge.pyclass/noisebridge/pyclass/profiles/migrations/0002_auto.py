...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing M2M table for field interest on 'UserProfile'
        db.delete_table('profiles_userprofile_interest')

        # Adding M2M table for field interests on 'UserProfile'
        db.create_table('profiles_userprofile_interests', (
        ...
        db.create_unique('profiles_userprofile_interests', ['userprofile_id', 'interest_id'])


    def backwards(self, orm):
        # Adding M2M table for field interest on 'UserProfile'
        db.create_table('profiles_userprofile_interest', (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('userprofile', models.ForeignKey(orm['profiles.userprofile'], null=False)),
            ('interest', models.ForeignKey(orm['profiles.interest'], null=False))
        ))
        db.create_unique('profiles_userprofile_interest', ['userprofile_id', 'interest_id'])

        # Removing M2M table for field interests on 'UserProfile'
        db.delete_table('profiles_userprofile_interests')
